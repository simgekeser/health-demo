...
export default class ActivityRecordsController extends React.Component {
  ...
  async endActivityRecord() {
    try {
      ...
      const activityRecordId = 'ActivityRecordRun235';
  ...
  async addActivityRecord() {
    try {
      ...
      const startTime = '2020-09-28 09:00:00';

      // Create end time that will be used to add activity record.
      const endTime = '2020-09-29 10:06:00';
      ...
      const activityRecord = {
        ...
        name: 'BeginActivityRecord',
        description: 'This is ActivityRecord begin test!',
        activityType: HmsActivityRecordsController.DT_CONTINUOUS_STEPS_DELTA,
        startTime: startTime,
        endTime: endTime,
        timeUnit: HmsActivityRecordsController.MILLISECONDS,
      };
      ...
      const dataCollector = {
        ...
        dataGenerateType: 0,
        dataCollectorName: 'AddActivityRecord',
      };
      ...
      const sampleSetMapArr = [
        {
          ...
          endTime: endTime,
          timeUnit: HmsActivityRecordsController.MILLISECONDS,
          fieldValue: HmsActivityRecordsController.FIELD_STEPS_DELTA,
          intValue: 1024,
        },
      ];
  ...
  async getActivityRecord() {
    try {
      ...
      const endTime = '2020-09-29 10:50:00';

      //Build dateMap with defined times above.
      const dateMap = {
        startTime: startTime,
        endTime: endTime,
        timeUnit: HmsActivityRecordsController.MILLISECONDS,
  ...
  /**
   * Registering an event to Monitor the Status Changes of Activity Records for RN Side.
   * </br>
   * The user can use addActivityRecordsMonitor to register an event to monitor the status changes of activity records.
   * When an activity record starts or stops, the caller will be notified via a broadcast.
   * The user can use removeActivityRecordsMonitor to unregister the event for activity records they no longer wish to monitor.
   * <p>
   * Note: The events can be listened via event name: "addActivityRecordsMonitor" on RN Side.
   * </p>
   ...
   * @returns {Promise<void>}
   */
  async addActivityRecordsMonitor() {
    try {
      Utils.logCall('addActivityRecordsMonitor - ActivityRecordsController');
      // Create start time that will be used to get activity record.
      // Call the related method in the HmsActivityRecordsController to get activity records
      const result = await HmsActivityRecordsController.addActivityRecordsMonitor();
      Utils.logResult('addActivityRecordsMonitor', result);
      Utils.notify('addActivityRecordsMonitor - ' + JSON.stringify(result));
    } catch (error) {
      Utils.logError(error);
    }
  }
  ...
  /**
   * Unregistering an event to Monitor the Status Changes of Activity Records for RN Side.
   * </br>
   * The user can use removeActivityRecordsMonitor to unregister the listener for activity records they no longer wish to monitor.
   * <p>
   * Note: The events can be removed via event name: "removeActivityRecordsMonitor" on RN Side.
   * </p>
   *
   * @returns {Promise<void>}
   */
  async removeActivityRecordsMonitor() {
    try {
      Utils.logCall('removeActivityRecordsMonitor - ActivityRecordsController');
      // Create start time that will be used to get activity record.
      // Call the related method in the HmsActivityRecordsController to get activity records
      const result = await HmsActivityRecordsController.removeActivityRecordsMonitor();
      Utils.logResult('removeActivityRecordsMonitor', result);
      Utils.notify('removeActivityRecordsMonitor - ' + JSON.stringify(result));
    } catch (error) {
      Utils.logError(error);
    }
  }
  ...
  async deleteActivityRecord() {
    try {
      ...
      // Create start time that will be used to delete activity record.
      const startTime = '2020-07-23 09:00:00';

      // Create end time that will be used to delete activity record.
      const endTime = '2020-07-23 10:00:00';
      ...
      <ScrollView style={styles.bg}>
        ...
        <View style={styles.innerBody}>
          ...
          <View style={styles.buttonDataController}>
            <TouchableOpacity
              style={styles.autoRecorderButton}
              onPress={() => this.addActivityRecordsMonitor()}
              underlayColor="#fff">
              <Text style={styles.smallButtonLabel}>
                addActivityRecordsMonitor
              </Text>
            ...
            <TouchableOpacity
              style={styles.autoRecorderButton}
              onPress={() => this.removeActivityRecordsMonitor()}
              underlayColor="#fff">
              <Text style={styles.smallButtonLabel}>
